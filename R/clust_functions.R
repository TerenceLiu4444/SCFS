...
GenerateSyntheticData <- function(n, p, s, k, signal_strength, noise_type) {
...
LloydIteration <- function(data, num_clusters, init_cluster_ids, num_iterations) {
...
SpectralClusterFeatureSelection <- function(data, num_clusters, init_cluster_ids, use_lloyd_iteration) {
  ...
  #' @return A vector containing the estimated cluster assignment.
  #' @references T. Liu, Y. Lu, B. Zhu, H. Zhao (2021). High-dimensional Clustering via Feature Selection with
  #' Applications to Single Cell RNA-seq Data.
  #' @examples
  #' synthetic_data <- GenerateSyntheticData(n=10, p=10, s=5, k=2, signal_strength=1, noise_type="gaussian")
  ...
  dist_mat = as.matrix(dist(data.filtered.svd$u[, 1:k]))
  scfs.result = list(cluster_ids = scfs.cluster_ids, dist_mat = dist_mat)
  return(scfs.result)
}
